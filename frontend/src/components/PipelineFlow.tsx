  ...
  return (
    <div className="flex flex-col h-full p-4 justify-center">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-white">Pipeline Flow</span>
        <span className={`text-xs px-2 py-0.5 rounded font-medium ml-auto ${isAlert ? "bg-red-500/10 text-red-400" : "bg-cyan-500/10 text-cyan-400"}`}>
          ...
          return (
            <div key={i} className="flex items-start flex-1 min-w-0">
              <div className="flex flex-col items-center gap-0.5 w-full">
                <div className={`w-3 h-3 rounded-full ${dotColor} transition-all duration-500 ${node.isCondition ? "rotate-45 rounded-sm" : ""}`} />
                <span className={`text-xs font-semibold whitespace-nowrap ${textColor}`}>{node.label}</span>
                {node.desc && <span className={`text-[10px] leading-tight ${descColor}`}>{node.desc}</span>}
      ...
      <div className="flex items-center mt-2">
        ...
        <div className="flex items-center gap-2 bg-slate-800/60 border border-slate-700/40 rounded-lg px-3 py-1.5">
          ...
          <span className={`text-xs font-semibold ${isAlert ? "text-cyan-300" : "text-slate-500"}`}>Agent</span>
          <span className={`text-[10px] ${isAlert ? "text-slate-400" : "text-slate-600"}`}>ReAct async</span>
          {agentResult && (
            <>
              <span className="text-slate-600">·</span>
              <span className={`text-[10px] font-medium ${agentResult.escalation_needed ? "text-red-400" : "text-cyan-400"}`}>
                {agentResult.escalation_needed ? "Escalated" : "OK"}
              </span>
              <span className="text-[10px] text-slate-500">{agentResult.actions_taken.length} calls</span>
            </>
          )}
          {!agentResult && isAlert && (
