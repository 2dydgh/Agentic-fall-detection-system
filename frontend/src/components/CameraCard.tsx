  ...
  return (
    <div className={`relative rounded overflow-hidden bg-g-bg border transition-all h-full ${borderClass}`}>
      ...
      <div className="absolute top-0 left-0 right-0 px-2 py-1 flex justify-between items-center z-10 bg-gradient-to-b from-black/70 to-transparent">
        <span className={`text-g-text font-medium ${compact ? "text-[9px]" : "text-[11px]"}`}>CAM {id} · {label}</span>
        <span className={`font-medium ${compact ? "text-[8px]" : "text-[10px]"} ${isAlert ? "text-g-red" : "text-g-green"}`}>
          {isAlert ? "ALERT" : "LIVE"}
        </span>
      </div>
      ...
      {isFallRecent && latest && (
        <>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10">
            {compact ? (
              <span className={`text-xs font-bold tracking-wider ${isAlert ? "text-g-red alert-text" : "text-g-orange"}`}>
                {severity} · {latest.score}
              </span>
            ) : (
              <>
                <span className={`text-lg font-bold tracking-widest drop-shadow-lg ${isAlert ? "text-g-red alert-text" : isMedium ? "text-g-orange" : "text-g-text"}`}>
                  FALL DETECTED
                </span>
                <div className="flex items-center gap-3 mt-2">
                  <span className={`text-sm font-semibold px-3 py-1 rounded-sm ${isAlert ? "bg-g-red text-white" : isMedium ? "bg-g-orange text-white" : "bg-g-blue text-white"}`}>
                    {severity}
                  </span>
                  <span className="text-white/90 text-sm font-medium font-mono bg-black/60 px-2 py-1 rounded-sm">
                    {latest.score}/100
                  </span>
                </div>
              </>
