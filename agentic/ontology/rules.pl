...
%
% 하한이 필요한 이유:
%   ActionNode 가 LOW 를 포함한 모든 판정을 DB 에 기록하고, PerceptionNode 는
%   같은 낙상의 재검출을 허용한다(COOLDOWN_FRAMES=60, 30fps 기준 약 2초).
%   하한이 없으면 사건 1건이 몇 초 전의 자기 자신을 '재낙상' 근거로 삼아
%   스스로를 HIGH 로 승격시키고 이메일·리포트까지 발송한다.
%
% 값의 근거:
%   재검출 간격(약 2초) 대비 충분한 여유를 두되, 임상적으로 의미 있는
%   재낙상은 살리는 선에서 5분으로 잡았다. 측정에 기반한 값이 아니라
%   공학적 판단이다.
%
% 이 하한이 포기하는 것:
%   5분 안에 일어난 '진짜' 2차 낙상은 r6/r13 을 발동시키지 못한다.
%   낙상 자체는 다른 규칙(r1, r5, r10 등)이 잡으므로 놓치지 않지만,
%   시간축 가중치는 붙지 않는다.
%
% 근본 해결:
%   재검출과 재낙상의 구분을 시간 간격으로 대리하고 있다. 제대로 하려면
%   track_id 로 동일 인물의 연속 이벤트를 묶어야 한다. 그때 이 하한은
%   불필요해진다.
repeat_fall_min_minutes(5).

rule(r6, high, I) :-
    repeat_fall_min_minutes(Min),
    prior_incident(_, _, M), M >= Min,
    no_movement_duration(I, S), S >= 10.
...
rule_text(r6,  '3일 내 동일 구역 재낙상(5분 이상 경과) + 무동작 10초 이상').
rule_text(r13, '3일 내 동일 구역 재낙상(5분 이상 경과)').
